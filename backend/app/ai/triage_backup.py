...
class TriageItem(BaseModel):
    ...
    forecast: Optional[ConversionForecast] = None  # New field for Phase 2.1 forecasting
...
# ---------------------------------------------------
# Phase 2.1: Individual Lead Forecasting Functions
# ---------------------------------------------------

def calculate_engagement_momentum(engagement: Dict[str, int], days_since_activity: Optional[int]) -> float:
    """Calculate engagement momentum (-1.0 to 1.0) based on recent activity patterns"""
    if not engagement or days_since_activity is None:
        return 0.0
    ...
    if total_engagement == 0:
        return -0.5 if days_since_activity > 30 else 0.0
    
    # Recent activity bonus (last 7 days)
    recent_activity = sum(v for k, v in engagement.items() if k in ['email_open', 'email_click', 'portal_login'])
    older_activity = total_engagement - recent_activity
    
    weighted_score = (recent_activity * recent_weight + older_activity * older_weight) / total_engagement
    
    # Normalize to -1.0 to 1.0
    momentum = (weighted_score - 0.5) * 2
...
def calculate_churn_risk(lead: Lead, triage_score: float, days_since_activity: Optional[int]) -> float:
    """Calculate churn risk (0.0 to 1.0) based on lead behavior and score"""
    risk_factors = []
    
    # High recency penalty
    if days_since_activity and days_since_activity > 30:
        risk_factors.append(0.4)
    elif days_since_activity and days_since_activity > 14:
        risk_factors.append(0.2)
    
    # Low engagement
    if not lead.engagement or sum(lead.engagement.values()) == 0:
    ...
    if (lead.source or "").lower() in ["paid_social", "unknown"]:
        risk_factors.append(0.15)
    
    # Calculate total risk (capped at 1.0)
    total_risk = min(1.0, sum(risk_factors))
    
    # Apply engagement momentum modifier
    momentum = calculate_engagement_momentum(lead.engagement, days_since_activity)
    if momentum > 0:
        total_risk *= (1.0 - momentum * 0.3)  # Reduce risk for positive momentum
    
    return max(0.0, min(1.0, total_risk))

def predict_conversion_timeline(triage_score: float, engagement_momentum: float, blockers: List[Blocker]) -> Tuple[Optional[int], Optional[int]]:
    """Predict days to conversion and next milestone"""
    
    # Base timeline based on score band
    base_timeline = {
        "hot": (7, 21),      # 1-3 weeks
        "warm": (21, 60),    # 3-8 weeks  
        "nurture": (60, 120), # 2-4 months
        "low": (120, 365)    # 4-12 months
    }
    ...
    if triage_score >= 85:
        band = "hot"
    elif triage_score >= 65:
        band = "warm"
    elif triage_score >= 45:
        band = "nurture"
    
    min_days, max_days = base_timeline[band]
    
    # Adjust based on engagement momentum
    if engagement_momentum > 0.5:
        min_days = int(min_days * 0.7)
        max_days = int(max_days * 0.8)
    elif engagement_momentum < -0.3:
        min_days = int(min_days * 1.5)
        max_days = int(max_days * 1.8)
    
    # Adjust based on blocker severity
    critical_blockers = sum(1 for b in blockers if b.severity == "critical")
    high_blockers = sum(1 for b in blockers if b.severity == "high")
    
    if critical_blockers > 0:
        min_days = int(min_days * 2.0)
        max_days = int(max_days * 2.5)
    elif high_blockers > 0:
        min_days = int(min_days * 1.3)
        max_days = int(max_days * 1.5)
    
    # Return average timeline and next milestone estimate
    avg_days = (min_days + max_days) // 2
    next_milestone_days = max(7, avg_days // 3)  # Next milestone is roughly 1/3 of total timeline
    
    return avg_days, next_milestone_days

def generate_conversion_path(lead: Lead, blockers: List[Blocker], triage_score: float) -> List[str]:
    """Generate personalized conversion path based on lead state"""
    path = []
    
    # Start with blocker resolution
    if blockers:
        critical_blockers = [b for b in blockers if b.severity == "critical"]
        high_blockers = [b for b in blockers if b.severity == "high"]
        
        if critical_blockers:
            path.append("🚨 Resolve critical blockers first")
        if high_blockers:
            path.append("⚠️ Address high-priority blockers")
    
    # Add engagement steps based on score
    if triage_score < 30:
        path.append("📧 Send re-engagement email sequence")
        path.append("📞 Make personal outreach call")
    elif triage_score < 60:
        path.append("🎯 Schedule personalized consultation")
        path.append("📚 Send targeted course materials")
    else:
        path.append("🎓 Move toward application process")
        path.append("📋 Schedule interview/assessment")
    
    # Add source-specific steps
    if (lead.source or "").lower() == "ucas_direct":
        path.append("📋 Guide through UCAS application")
    elif (lead.source or "").lower() == "school_tour":
        path.append("🏫 Follow up on campus visit experience")
    elif (lead.source or "").lower() == "referral":
        path.append("🤝 Leverage referral relationship")
    
    return path

def forecast_individual_lead(
    lead: Lead, 
    triage_score: float, 
    triage_band: str,
    blockers: List[Blocker],
    ...
    """Generate comprehensive conversion forecast for individual lead"""
    
    # Calculate key metrics
    engagement_momentum = calculate_engagement_momentum(lead.engagement, days_since_activity)
    churn_risk = calculate_churn_risk(lead, triage_score, days_since_activity)
    
    # Predict timeline
    days_to_conversion, days_to_next_milestone = predict_conversion_timeline(
        triage_score, engagement_momentum, blockers
    )
    
    # Generate conversion path
    conversion_path = generate_conversion_path(lead, blockers, triage_score)
    
    # Determine confidence band
    confidence_band = "medium"
    if triage_score >= 80 and engagement_momentum > 0.3:
        confidence_band = "high"
    elif triage_score < 40 or churn_risk > 0.6:
        confidence_band = "low"
    
    # Calculate conversion probability (base on score, adjusted by momentum and risk)
    base_probability = triage_score / 100.0
    momentum_adjustment = engagement_momentum * 0.2
    risk_adjustment = (1.0 - churn_risk) * 0.3
    
    conversion_probability = max(0.0, min(1.0, 
        base_probability + momentum_adjustment + risk_adjustment
    ))
    ...
    if engagement_momentum < -0.3:
        risk_factors.append("Declining engagement")
    if not lead.has_email and not lead.has_phone:
        risk_factors.append("No contact information")
    if days_since_activity and days_since_activity > 30:
        risk_factors.append("Long period of inactivity")
    
    # Determine next milestone
    next_milestone = "Initial engagement"
    if triage_score >= 60:
        next_milestone = "Application preparation"
    elif triage_score >= 40:
        next_milestone = "Course consultation"
